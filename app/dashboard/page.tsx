...
import CryptoBinanceLayout from "@/simulation/components/CryptoBinanceLayout";
...
function DashboardInner() {
  ...
  function handleBuy(stopLoss?: number, takeProfit?: number) {
    ...
    setTimeout(() => { isTradingRef.current = false; }, 1000);

    const buyPrice = liveTick.inrPrice;
    const ok = wallet.buy(selected, qty, buyPrice, stopLoss, takeProfit);
  ...
  function handleSellClick() {
    ...
    const isCrypto = selectedStock.currency === "USD";
    if (isCrypto) {
      if (qty <= 0) {
        setTradeMsg({ text: "\u274C Enter a valid quantity to sell.", ok: false });
        setTimeout(() => setTradeMsg(null), 4000);
        return;
      }
      const lot = symbolLots[0];
      const sellQty = Math.min(qty, lot.remainingQty);
      handleSellLot(lot.lotId, sellQty);
      return;
    }
  ...
  function handleCoverClick() {
    ...
    if (isCrypto) {
      if (qty <= 0) {
        setTradeMsg({ text: "\u274C Enter a valid quantity to cover.", ok: false });
        setTimeout(() => setTradeMsg(null), 4000);
        return;
      }
      const pos = shortPositions[0];
      const coverQty = Math.min(qty, pos.remainingQty);
      handleCoverShort(pos.positionId, coverQty);
      return;
    }
  ...
  function handleOpenShort(stopLoss?: number, takeProfit?: number) {
    if (!liveTick.isLive) return;
    if (isTradingRef.current) return;
    isTradingRef.current = true;
    setTimeout(() => { isTradingRef.current = false; }, 1000);

    const ok = wallet.openShort(selected, qty, liveTick.inrPrice, stopLoss, takeProfit);
  ...
  return (
    <div className={`bg-gray-950 text-white flex flex-col ${usdLocale ? "min-h-screen overflow-y-auto" : "h-screen overflow-hidden"}`}>
      ...
      {/* Stock tabs - always visible for switching between NSE and crypto */}
      <div className="flex gap-1.5 overflow-x-auto flex-shrink-0 px-4 py-2" style={{ scrollbarWidth: "thin", scrollbarColor: "#374151 transparent", background: "#111827" }}>
        {[
      ...
      {usdLocale ? (
        <CryptoBinanceLayout
          ...
          handleOpenShort={handleOpenShort}
          ...
          <div className="flex flex-col flex-1 p-4 gap-3 min-w-0">
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-4 flex-shrink-0">
              ...
              <div className="flex items-center gap-3">
                <div className="flex rounded-lg overflow-hidden flex-shrink-0" style={{ border: "1px solid #374151" }}>
                  <button
                    onClick={() => setTradeMode("long")}
                    className="px-3 py-1.5 text-xs font-bold transition"
                    style={{ background: tradeMode === "long" ? "#22c55e" : "#1f2937", color: tradeMode === "long" ? "#000" : "#6b7280" }}
                  >{'\u25B2'} Long</button>
                  <button
                    onClick={() => setTradeMode("short")}
                    className="px-3 py-1.5 text-xs font-bold transition"
                    style={{ background: tradeMode === "short" ? "#ef5350" : "#1f2937", color: tradeMode === "short" ? "#fff" : "#6b7280", borderLeft: "1px solid #374151" }}
                  >{'\u25BC'} Short</button>
                </div>
                <div className="flex items-center gap-2">
                ...
                <span className={`text-xs ${tradeCost > wallet.state.balance ? "text-red-400" : "text-gray-600"}`}>
                  {tradeCost > wallet.state.balance ? `\u26A0 Need \u20B9${(tradeCost - wallet.state.balance).toFixed(2)} more` : `Balance: \u20B9${wallet.state.balance.toFixed(2)}`}
                </span>
                <button
                  onClick={() => { if (tradeMode === "long") handleBuy(); else handleOpenShort(); }}
                  ...
                  style={{ background: tradeMode === "long" ? "#22c55e" : "#ef5350" }}
                >{tradeMode === "long" ? "BUY \u2014 New Lot" : "SHORT \u2014 Sell to Open"}</button>
                {tradeMode === "short" ? (
                  <button onClick={handleCoverClick} disabled={wallet.getOpenShorts(selected).length === 0 || !marketStatus.open}
                    className="flex-1 font-bold py-2 rounded-lg transition disabled:opacity-40 disabled:cursor-not-allowed text-white"
                    style={{ background: "#6366f1" }}>COVER</button>
                ) : (
                  <button onClick={handleSellClick} disabled={holding.length === 0 || !marketStatus.open}
                    className="flex-1 bg-red-600 hover:bg-red-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-2 rounded-lg transition">SELL</button>
                )}
              </div>
              {tradeMsg && (
                <div className={`mt-3 text-sm px-3 py-2 rounded-lg ${tradeMsg.ok ? "bg-green-900/40 border border-green-800/40 text-green-300" : "bg-red-900/40 border border-red-800/40 text-red-300"}`}>
                  {tradeMsg.text}
                </div>
              )}
            </div>
              ...
              {wallet.getAllOpenLots().length === 0 && wallet.getAllOpenShorts().length === 0 ? (
                ...
                <div className="space-y-2">
                  {wallet.getAllOpenLots().map(lot => (
                    <HoldingLot key={lot.lotId} lot={lot}
                      onSell={lot => { handleSymbolSelect(lot.symbol); setInitialSellLot(lot); setIsSellModalOpen(true); }}
                    />
                  ))}
                  {wallet.getAllOpenShorts().map(pos => (
                    <ShortPositionCard key={pos.positionId} position={pos}
                      onCover={(p) => { setSelected(p.symbol); setQty(1); setTradeMsg(null); setSellLotTarget({ lotId: p.positionId, symbol: p.symbol, buyPrice: p.shortPrice, originalQty: p.originalQty, remainingQty: p.remainingQty, buyTimestamp: p.openTimestamp, isClosed: p.isClosed }); }}
                    />
                  ))}
                </div>
              )}
                  ...
                  <div className="space-y-2">
                    {all.map(t => (
                      <div key={t._id} className="bg-gray-800 rounded-lg p-2 text-xs">
                        ...
                        <div className="text-gray-600 mt-0.5">{new Date(t.timestamp).toLocaleTimeString("en-IN")}</div>
                      </div>
                    ))}
                  </div>
                );
